...
func HandleDBUpdate(c *gin.Context) {
	...
	columnsNames, err := columns.GetUpdateColumns(sqlCommand)
	if err != nil {
		c.JSON(http.StatusOK, response.ErrorResponse{Code: 20202, Message: "Bad SQL statement"})
		return
	}
	...
	switch result {
	case true:
		columnsCanUpdate, err := services.GetColumnsRole(callerPrivateKey, callerAddress, tableName)
		if err != nil {
			c.JSON(http.StatusOK, response.ResponseInternalError())
			return
		}
		columnsAuth := utils.CompareColumns(columnsCanUpdate, columnsNames)
		...
		case true:
			...
			err = services.Consume(callerPrivateKey, callerAddress, viper.GetInt64(`PRICE.UPDATE`))
			if err != nil {
